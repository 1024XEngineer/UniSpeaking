...
只有前三个模块可以作为移动端定稿实现继续联调。IELTS、英文面试和个人主页中的现有
页面、路由及演示数据只是阶段性原型，后续仍需参考 Web 端交互继续开发，不能按完成
模块交接。
...
- IELTS 与英文面试的移动端前端尚未定稿，后端也尚未开发。目前相关页面只能作为阶段性原型/占位体验，不能作为完成模块或真实数据功能交付。
...
- 全量 ESLint 仍有一些较早存在的动画 Hook 告警，主要分布在：
  ...
  - `AuthScreens.tsx`
  - `ScenesScreen.tsx`
- IELTS、英文面试和个人主页的移动端前端尚未定稿，需要继续开发。
- IELTS 和英文面试目前没有后端，相关数据均为演示数据。
- 当前工程已纳入 UniSpeaking 单仓库，位于 `frontend/mobile`。

## 8. 接手后的优先顺序

1. 保持自由对话、场景广场和学习资产三个已定稿模块的交互与视觉稳定。
2. 参考 Web 端继续完成 IELTS 移动端模块并定稿。
3. 参考 Web 端继续完成英文面试移动端模块并定稿。
4. 参考 Web 端继续完成个人主页移动端模块并定稿。
5. 保持 IELTS、面试入口独立，在联调完成前明确使用占位数据。
6. 将 `src/data` 中的演示数据逐步替换为 service/repository 层接口。
7. 每次改动至少运行 `npx tsc --noEmit`；用户要求时再部署到实体机。
...
- 不要把 IELTS、英文面试和个人主页的阶段性移动端页面误报为已定稿。
