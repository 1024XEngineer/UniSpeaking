...
function Overview({ calendar, statistics, onMonthChange }) {
  const weeklyMinutes = Math.floor((statistics?.weeklyPracticeSeconds || 0) / 60);
  ...
  return <><PageHeader eyebrow="PERSONAL OVERVIEW" title="你的学习空间" subtitle="把每一次开口变成看得见、可继续的成长记录。" action={calendar?.checkedInToday ? <span className="today-checkin"><CalendarCheck2 />今日已打卡</span> : null} /><div className="stat-grid"><article><Clock /><span><small>本周学习时长</small><strong>{weeklyMinutes} <em>分钟</em></strong></span></article><article><BookOpenText /><span><small>已保存学习资产</small><strong>{trainingRecordCount} <em>项</em></strong></span></article><article><Fire /><span><small>连续学习天数</small><strong>{consecutiveLearningDays} <em>天</em></strong></span></article></div><section className="overview-grid"><LearningCalendar calendar={calendar} onMonthChange={onMonthChange} /><article className="rhythm-card"><p className="eyebrow">LAST SEVEN DAYS</p><h2>练习节奏</h2><div className={cx("bars", !days.length && "is-empty")}>{days.length ? days.map((day) => { const seconds = day.practiceSeconds || 0; const minutes = Math.floor(seconds / 60); const height = maximumSeconds ? Math.max(3, (seconds / maximumSeconds) * 100) : 3; const weekday = new Date(`${day.date}T00:00:00+08:00`).toLocaleDateString("zh-CN", { weekday: "short", timeZone: "Asia/Shanghai" }); return <span key={day.date} className={seconds ? "" : "is-zero"} style={{ height: `${height}%` }} aria-label={`${day.date}，练习 ${minutes} 分钟`} title={`${day.date} · ${minutes} 分钟`}><small>{minutes}m</small><em>{weekday}</em></span>; }) : <p>完成一次至少 3 分钟的练习后，这里会显示你的节奏。</p>}</div></article></section><AchievementSystem /></>;
}
