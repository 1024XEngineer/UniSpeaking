...
@Service
public class CustomSceneGenerationServiceImpl implements CustomSceneGenerationService {

	private static final Logger LOGGER =
			LoggerFactory.getLogger(CustomSceneGenerationServiceImpl.class);
	...
	public CustomSceneGenerationServiceImpl(
	...
	private String buildPrompt(
		...
		return """
				...
				Required outcomes must contain
				3 to 8 observable learner actions. minimum_user_turns must be 3 to 20;
				maximum_user_turns must be greater than it and no more than 30.
				estimated_minutes must be an integer from 5 to 30 and should reflect the
				difficulty and number of learning items.
	...
	private String parseSuccessFactor(JsonNode node) {
		...
		int estimatedMinutes = requiredInteger(node, "estimated_minutes", 5, 30);
		int minimumTurns = requiredInteger(node, "minimum_user_turns", 3, 20);
		int maximumTurns = requiredInteger(node, "maximum_user_turns", 4, 30);
