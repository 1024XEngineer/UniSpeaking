...
public interface IeltsSceneService
		extends SceneService<IeltsGenerationRequest, IeltsGenerationResponse> {

	/** 覆写通用场景生成方法，返回 IELTS 练习生成结果。 */
	@Override
	...
	/** 获取当前用户拥有的 IELTS 练习，无权限或不存在时抛出异常。 */
	IeltsPracticeRecord requireOwnedPractice(String ieltsId);
